...
double computeMobility(Graph &g, bool usePrevDistrib, std::string folder, int u)
{
    ...
    do
    {
        ...
        if(nbIterTot % nbIterUpdate == 0)
        {
            ...
            //errOccup = errP;
            mobility /= pTemp * (g.m_site.size() / (float) repetition) * ( g.m_field.norm() );
...
{
    Graph g;
    g.m_system.m_boxSize.m_x = (dim)*latticeParam;
    g.m_system.m_boxSize.m_y = (dim)*latticeParam;
    g.m_system.m_boxSize.m_z = (dim)*latticeParam;
                    ...
                    for (int i(0) ; i < dim ; i ++)
                    {
                        for (int j(0) ; j < dim ; j++)
                        {
                            ...
                            for (int k(0) ; k < dim ; k++)
                            {
                                ...
                                // Set position
                                center.m_x = i*latticeParam + a * g.m_system.m_boxSize.m_x;
                                center.m_y = j*latticeParam + b * g.m_system.m_boxSize.m_y;
                                center.m_z = k*latticeParam + c * g.m_system.m_boxSize.m_z;
                                rf.m_center = center;
    ...
    g.m_system.m_boxSize.m_x = 2 * (dim)*latticeParam;
    g.m_system.m_boxSize.m_y = 2 * (dim)*latticeParam;
    g.m_system.m_boxSize.m_z = 2 * (dim)*latticeParam;
    g.m_system.m_topo.m_chainLength = dim;
...
void testCubic()
{
   Graph g = createCubic(5,1);
    ...
    double sigma(0.1);
    g.gdm(sigma,0, (unsigned int) time(NULL), true);
...
void computeMobilityDependance(Graph &g, std::vector<double> pField, std::vector<double> pTemperature, std::vector<double> pConcentration, std::string folder, RateType rate)
{
...
Graph initCoarseGraph(std::string fileNameCoord, std::string configFile)
{
    /// Read morphology from pdb file ///
    Coarsegrain morpho;
    morpho.readSystem(fileNameCoord);
    morpho.initGeom();
    morpho.arrangeCrystalMol();
    ...
    /// Create the graph, using info from config file ///
    Graph g;
    g.m_system = morpho;
    ifstream is(configFile);
    if(is.is_open())
    ...
    g.m_temperature = 300;
    g.findNeighbors();
    g.writeGraph("test.pdb","test.psf",false);
    g.computeRate(maPheno);
