...
int main(int argc, char *argv[])
{
    ...
    if( (argc < 5 && (mode != "prep" && mode != "coarse") ) || (argc < 4 && mode == "prep") || (argc < 3 && mode == "coarse") )
    {
    ...
    if(mode == "mob")
    {
        ...
        computeMobilityDependance(g, pField, pTemperature, pConcentration ,folder);
    }
    ...
    if(mode == "transfer_geom")
    {
        ...
        g.writeTransferDistanceAngle(transferGeomFile,0,0);
    }

    /// Calculate morphological properties of a coarse grained MD ///
    if(mode == "coarse")
    {
        systemMorpho(fileNameCoord);
    }
