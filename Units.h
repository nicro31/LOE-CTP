...
struct Vect3
{
    ...
    static double distancePbc(const Vect3 &v1, const Vect3 &v2, const double &boxSize, bool &periodic)
	{
	    periodic = false;
	    double dxabs(fabs(v2.m_x-v1.m_x));
	    double dx = (std::min(dxabs, boxSize - dxabs ));
	    if (dx == boxSize - dxabs) {periodic = true;}
	    double dyabs(fabs(v2.m_y-v1.m_y));
	    double dy = (std::min(dyabs, boxSize - dyabs ));
	    if (dy == boxSize - dyabs) {periodic = true;}
        double dzabs(fabs(v2.m_z-v1.m_z));
	    double dz = (std::min(dzabs, boxSize - dzabs ));
	    if (dz == boxSize - dzabs) {periodic = true;}
        return ( sqrt( dx*dx + dy * dy + dz *dz ) );
	}

    // Displacement vector with periodic boundary conditions
    static Vect3 displacementPbc(const Vect3 &v1, const Vect3 &v2, const double &boxSize)
	{
	    Vect3 v;

	    double dxabs(fabs(v2.m_x-v1.m_x));
	    if(dxabs < boxSize - dxabs)
        {
            v.m_x = v2.m_x - v1.m_x;
        }
        else
        {
            if ( v2.m_x > v1.m_x ) { v.m_x = -(boxSize - dxabs); }
            else { v.m_x = (boxSize - dxabs); }

        }

	    double dyabs(fabs(v2.m_y-v1.m_y));
        if(dyabs < boxSize - dyabs)
        {
            v.m_y = v2.m_y - v1.m_y;
        }
        else
        {
            if ( v2.m_y > v1.m_y) { v.m_y = -(boxSize - dyabs); }
            else { v.m_y = (boxSize - dyabs); }

        }

        double dzabs(fabs(v2.m_z-v1.m_z));
        if(dzabs < boxSize - dzabs)
        {
            v.m_z = v2.m_z - v1.m_z;
        }
        else
        {
            if ( v2.m_z > v1.m_z ) { v.m_z = -(boxSize - dzabs); }
            else { v.m_z = (boxSize - dzabs); }
    ...
    static Vect3 displacementPbc(const Vect3 &v1, const Vect3 &v2, const double &boxSize, Vect3 &pbc)
	{
	    Vect3 v;
        ...
        {
            if ( v2.m_x > v1.m_x ) { v.m_x = -(boxSize - dxabs); pbc.m_x = -1; }
            else { v.m_x = (boxSize - dxabs); pbc.m_x = 1; }
        ...
        {
            if ( v2.m_y > v1.m_y) { v.m_y = -(boxSize - dyabs); pbc.m_y = -1;}
            else { v.m_y = (boxSize - dyabs); pbc.m_y = 1;}
        ...
        {
            if ( v2.m_z > v1.m_z ) { v.m_z = -(boxSize - dzabs); pbc.m_z = -1;}
            else { v.m_z = (boxSize - dzabs); pbc.m_z = 1;}
    ...
    static Vect3 siteDisplacementPbc(const Vect3 &v1, const Vect3 &v2, const double &boxSize, const Vect3 &pbc)
	{
	    Vect3 v;
        ...
        {
            if ( pbc.m_x == -1 ) { v.m_x = -(boxSize - dxabs); }
            else { v.m_x = (boxSize - dxabs); }
        ...
        {
            if ( pbc.m_y == -1) { v.m_y = -(boxSize - dyabs); }
            else { v.m_y = (boxSize - dyabs); }
        ...
        {
            if ( pbc.m_z == -1 ) { v.m_z = -(boxSize - dzabs);}
            else { v.m_z = (boxSize - dzabs);}
...
bool crossingRaySphere(const Vect3 &rf1,const Vect3 &rf2,const Vect3 &rfInfluence,const double &sphereR, const double &boxSize);
