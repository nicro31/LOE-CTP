...
void Graph::writeGraph(std::string fileNameGraphPdb, std::string fileNameGraphPsf, bool periodic)
{
    ...
    if (outFlux.is_open())
    {
        ...
        outFlux << '\n';
        outFlux << "CRYST1";
        outFlux << std::setw(9) << m_system.m_boxSize;
...
void Graph::writeCurrent(std::string fileNameOccupancyPdb, std::string fileNameCurrentPdb, std::string fileNameCurrentPsf, bool periodic)
{
    ...
    if (outFlux0.is_open())
    {
        ...
        outFlux0 << '\n';
        outFlux0 << "CRYST1";
        outFlux0 << std::setw(9) << m_system.m_boxSize;
...
void Graph::gdm(double sigma, double mu, unsigned int seed)
{
    // Init gaussian
...
void Graph::writeTransferDistanceAngle(std::string fileName, int l1, int l2)
{
        ...
        for(int i(0) ; i < m_site.size() ; i++)
        {
            for (int j(0) ; j < m_site[i].m_neighbor.size() ; j++ )
            {
                ...
                if( h == inter )
                {
                    ...
                    outFlux << d << "   " << dMin << "   " << angleN << "    " << angleP  << "    " << dangleN << "    " << dangleP << "   " << angleNLoc << "    " << anglePLoc  << "    " << dangleNLoc << "    " << danglePLoc <<  "    " << transfer << "     " << i << "    " << m_site[i].m_neighborIdx[j] << '\n';
                }
